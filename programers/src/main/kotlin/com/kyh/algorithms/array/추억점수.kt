...
class 추억점수 {
    fun solution(name: Array<String>, yearning: IntArray, photo: Array<Array<String>>): IntArray {
        ...
        val yearningMap = name.indices.associate {
            name[it] to yearning[it]
        }

        photo.forEachIndexed { idx, photos ->
            var sum = 0
            photos.forEach { sum += yearningMap.getOrDefault(it, 0) }
            answer[idx] = sum
        }
...
fun main() {
    ...
    val answer = problem.solution(
        ...
        yearning = arrayOf(5, 10, 1, 3).toIntArray(),
        photo = arrayOf(
