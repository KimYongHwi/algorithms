...
players	(0번째 인덱스가 1등, 마지막 인덱스가 꼴등)
...
class 달리기경주 {
    fun solution(players: Array<String>, callings: Array<String>): Array<String> {
        var answer = players
        val playerMap = players.indices.associateBy { players[it] }.toMutableMap()

        callings.forEach { calling ->
            val curIdx = playerMap[calling]!!
            val prevIdx = curIdx - 1
            val prevPlayer = answer[prevIdx]

            answer[prevIdx] = calling
            answer[curIdx] = prevPlayer

            playerMap[calling] = prevIdx
            playerMap[prevPlayer] = curIdx
        }
